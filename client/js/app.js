    ...
    var App = Class.extend({
        ...
        start: function(username) {
            ...
            if (username && !this.game.started) {
                const config = (window.config && window.config.build) ? window.config.build : {};
                this.config = config;
        
                const host = window.location.hostname;
                const port = window.location.port;
        
                log.debug("Starting game with dynamic config.");
                this.game.setServerOptions(host, port, username);
        
                // ✅ Important : redirection simple sans aucun paramètre autre que le nom
                window.location.href = '/?player-name=' + encodeURIComponent(username);
            }
        }
